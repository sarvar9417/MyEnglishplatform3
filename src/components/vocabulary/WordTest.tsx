...
export default function WordTest({ word, allWords, onAnswer }: Props) {
  ...
  function handlePick(opt: string) {
    ...
    setRevealed(true)
    setTimeout(() => {
      onAnswer(opt === word.uzbek)
      setSelected(null)
      setRevealed(false)
    }, 1200)
  }

  const isCorrect = selected === word.uzbek
  ...
  return (
    <div className="w-full">
      <div className="text-center mb-4">
        ...
        <p className="text-2xl sm:text-3xl font-bold text-gray-900 mt-2">{word.english}</p>
        <p className="text-sm text-gray-400 mt-1">To'g'ri tarjimani toping</p>
      </div>
