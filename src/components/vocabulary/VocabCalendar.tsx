    ...
    <div className="space-y-3">
      ...
      <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <button onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 transition-all">
            <ChevronLeft size={18} />
          </button>
          <span className="text-sm font-bold text-gray-800">
            {MONTH_NAMES[month]} {year}
          </span>
          <button onClick={nextMonth} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 transition-all">
        ...
        <div className="grid grid-cols-7 gap-0.5 sm:gap-1 text-center">
          {WEEKDAYS.map(d => (
            <div key={d} className="text-[9px] sm:text-[10px] font-semibold text-gray-400 py-1">{d}</div>
          ))}
          {days.map((day, i) => {
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isFuture ? 'opacity-20 cursor-default' : 'hover:bg-gray-100 cursor-pointer'}
                  ${isSel ? 'ring-2 ring-b1-500 bg-b1-50' : ''}
                  ${isToday && !isSel ? 'ring-1 ring-b1-300' : ''}
                `}
              >
                <span className={`block leading-tight ${isToday ? 'text-b1-600 font-bold' : 'text-gray-700'}`}>
                  {day}
                </span>
                {dotClass && (
      ...
      {selectedDate && (
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-b1-50 flex items-center justify-center">
              <CalendarDays size={16} className="text-b1-600" />
            </div>
            <div className="flex-1">
              <p className="text-sm font-semibold text-gray-900">{fmt(selectedDate)}</p>
              {selectedSession ? (
          ...
          {selectedDate !== today && (
            <button onClick={onClose} className="mt-2 w-full py-2 text-xs text-gray-500 hover:text-gray-700 transition-all">
