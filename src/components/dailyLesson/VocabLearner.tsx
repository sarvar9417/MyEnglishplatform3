...
const RULE_LABELS: Record<string, { label: string; color: string }> = {
  qisqa:   { label: 'Qisqa (-er/-est)', color: 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300' },
  uzun:    { label: 'Uzun (more/most)', color: 'bg-violet-100 dark:bg-violet-900/40 text-violet-700 dark:text-violet-300' },
  'y bilan': { label: '-y → -ier/-iest', color: 'bg-teal-100 dark:bg-teal-900/40 text-teal-700 dark:text-teal-300' },
  'e bilan': { label: '-e → -r/-st',   color: 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300' },
  cvc:     { label: 'CVC (undosh×2)',  color: 'bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300' },
  notogri: { label: 'Noto\'g\'ri',     color: 'bg-rose-100 dark:bg-rose-900/40 text-rose-700 dark:text-rose-300' },
}
...
export default function VocabLearner({ vocab, addXP, lessonId, lessonLevel, onVocabDone }: {
        ...
        {reviewWords.length > 0 && (
          <div className="rounded-xl border border-violet-100 dark:border-violet-800 bg-gradient-to-r from-violet-50 to-indigo-50 dark:from-violet-900/20 dark:to-indigo-900/20 p-3 animate-slide-up">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold text-violet-600 dark:text-violet-400 uppercase tracking-wider">Takrorlash uchun</span>
              <span className="text-[11px] text-violet-400 dark:text-violet-500">{reviewWords.length} ta so'z</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {reviewWords.map((w, i) => (
                <div key={i} className="flex items-center gap-1.5 bg-white dark:bg-gray-800 rounded-lg px-2.5 py-1.5 border border-violet-100 dark:border-violet-700">
                  ...
                  <span className="text-[11px] text-gray-400 dark:text-gray-500">— {w.uz}</span>
                </div>
              ))}
            </div>
          ...
          return (
            <div key={g.rule}>
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-xs text-gray-400 dark:text-gray-500">{g.words.length} ta so'z</span>
              </div>
              <div className="overflow-x-auto rounded-xl border border-gray-100 dark:border-gray-700">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-gray-800 text-left text-[11px] text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  ...
                  <tbody>
                    {g.words.map((v, i) => (                        <tr key={i} className="border-t border-gray-50 dark:border-gray-800">
                        ...
                        <td className="py-1.5 px-3 text-gray-400 dark:text-gray-500 italic text-[11px] hidden md:table-cell">{v.example}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className="space-y-4">
        ...
        <div onClick={() => setFlipped(!flipped)} className="cursor-pointer select-none">
          <div className={`relative w-full min-h-[200px] rounded-2xl border-2 transition-all duration-300 flex flex-col items-center justify-center p-3 sm:p-6 ${
            ...
            <div className="text-center">
              {!flipped ? (
                <>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">🇬🇧 Inglizcha</p>
                  <div className="flex items-center justify-center gap-2 mb-2">
                    <p className="text-3xl font-bold text-gray-900 dark:text-gray-100">{word.en}</p>
                    <AudioButton text={word.en} size="md" />
                  </div>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-4">👆 bosing — tarjimasini ko'rish</p>
                </>
              ) : (
                <>
                  ...
                  <div className="mt-3 flex items-center justify-center gap-2">
                    <AudioButton text={word.en} size="sm" label="Qayta tinglash" />
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-4">👆 bosing — yana yashirish</p>
                </>
              )}
            </div>
          </div>
        ...
        {knownIds.size === vocab.length && (
          <div className="card bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30 border-green-200 dark:border-green-800 text-center py-4 animate-slide-up">
            <p className="font-bold text-green-700 dark:text-green-400 text-lg">🎉 Tabriklaymiz!</p>
            <p className="text-sm text-green-600 dark:text-green-400">Barcha {vocab.length} ta so'zni o'zlashtirdingiz!</p>
            <div className="flex items-center justify-center gap-2 mt-3 flex-wrap">
              <button onClick={startMiniQuiz} className="btn-primary text-sm">🧠 Mini Quiz (5 savol)</button>
              <button onClick={startTest} className="btn-secondary text-sm">📝 To'liq test</button>
            </div>
          </div>
        )}
      </div>
  ...
  if (mode === 'vocabTest') {
    if (vocab.length === 0) return <div className="card text-center py-6 text-gray-500 dark:text-gray-400">Bu darsda lug'at so'zlari mavjud emas.</div>
          ...
          <div className="card text-center py-6 bg-gradient-to-b from-green-50 dark:from-green-900/30 to-white dark:to-gray-800 border-green-200 dark:border-green-800">
            ...
            {!srsPushed ? (
              <button onClick={pushToSRS} disabled={srsPushing}
                ...
                {srsPushing ? 'Saqlanmoqda...' : '📚 SRS ga saqlash (takrorlash uchun)'}
              </button>
            ) : (
              <div className="mt-3 flex items-center justify-center gap-2 text-sm text-green-600 dark:text-green-400 font-semibold animate-slide-up">
                <CheckCircle size={16} /> SRS ga saqlandi! Endi bu so'zlar takrorlashda ko'rinadi
              </div>
            )}

            <div className="flex items-center justify-center gap-3 mt-3">
              <button onClick={retakeTest} className="btn-secondary text-sm"><RotateCcw size={14} /> Qayta boshlash</button>
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          {miniQuizMode ? (
            <p className="text-xs font-semibold text-violet-500 dark:text-violet-400">🧠 Mini Quiz</p>
          ) : (
            <p className="text-xs text-gray-500 dark:text-gray-400">{testSectionIdx + 1} / {testSections.length} bo'lim</p>
          )}
        ...
        {currentSectionQuestions.length === 0 ? (
          ...
          <>
            <div className="rounded-2xl border-2 border-primary-200 dark:border-primary-800 bg-primary-50 dark:bg-primary-900/30 p-3 sm:p-6 text-center">
              {miniQuizMode ? (
                <p className="text-xs text-violet-400 dark:text-violet-300 mb-2 font-semibold">🧠 Mini Quiz — so'zlarni tekshiring</p>
              ) : (
              ...
              {!miniQuizMode && testSectionIdx === 2 && <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1 italic">Bo'sh joyga mos so'zni toping</p>}
            </div>
            ...
            {testSubmitted[testIdx] && (
              <div className={`card text-center py-3 ${testResults[testIdx] ? 'bg-green-50 dark:bg-green-900/30 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/30 border-red-200 dark:border-red-800'}`}>
                ...
                {!testResults[testIdx] && <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">To'g'ri javob: <strong>{currentQ.correct}</strong></p>}
                <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1 italic">"{currentQ.word.example}"</p>
                <button onClick={nextTest} className="btn-primary mt-3 text-sm">
                  {testIdx < currentSectionQuestions.length - 1 ? 'Keyingi savol →' :
                   testSectionIdx < testSections.length - 1 ? `Keyingi bo'lim →` : '🏁 Yakunlash'}
                </button>
              </div>
            )}
          </>
