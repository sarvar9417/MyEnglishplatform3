  ...
  return (
    <div className="fixed top-4 right-4 left-4 sm:left-auto z-[9999] flex flex-col gap-2 max-w-sm w-full pointer-events-none">
      {toasts.map((t) => (
        <div
          key={t.id}
          className={`pointer-events-auto flex items-start gap-2 px-4 py-3 rounded-xl border shadow-lg text-sm font-medium animate-fade-in-right ${TYPE_STYLES[t.type]}`}
        >
          <span className="mt-0.5">{TYPE_ICONS[t.type]}</span>
