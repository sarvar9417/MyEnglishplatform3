  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <div className={`badge text-xs ${TAG_COLORS[topic.tag] ?? 'bg-gray-100 text-gray-600'}`}>
          {topic.tag}
        </div>
      </div>
  ...
  return (
    <div className="p-3 sm:p-6 max-w-3xl mx-auto space-y-4 sm:space-y-5">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <span className={`badge ${TAG_COLORS[topic.tag] ?? ''}`}>{topic.tag}</span>
      </div>
    ...
    <div className={`p-4 rounded-2xl border transition-colors ${
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {ex.options.map((opt, i) => {
          ...
          if (submitted) {
            if (isCorrectOpt) cls = 'border-green-400 bg-green-100 text-green-800 font-bold'
            else if (isSelected && !isCorrectOpt) cls = 'border-red-400 bg-red-100 text-red-700'
...
export default function Grammar() {
  ...
  return (
    <div ref={topRef} className="h-full overflow-y-auto scrollbar-hide">
      {/* ── Topic select ── */}
      {phase === 'select' && (
        loading ? (
