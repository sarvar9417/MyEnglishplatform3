  ...
  return (
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center gap-2">
        {fromSkills && (
          <button onClick={() => navigate('/skills')} className="btn-ghost p-1.5 rounded-lg -ml-1.5" aria-label="Ko'nikmalarga qaytish">
            <ChevronLeft size={16} />
          </button>
        )}
        ...
        <h1 className="text-base sm:text-lg font-bold text-gray-900">Vocabulary</h1>
      </div>
      <div className="flex items-center gap-1">
        <button onClick={() => setShowTypingGame(true)} className="btn-secondary p-2 rounded-lg" title="O'yin">
          <span className="text-sm">🎮</span>
        </button>
        <button onClick={() => setShowSentenceGame(true)} className="btn-secondary p-2 rounded-lg" title="Gap tarjima">
