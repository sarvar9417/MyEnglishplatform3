      ...
      {segments.map((seg, i) =>
        seg.vocab ? (
          <mark
            ...
            className="bg-yellow-200 hover:bg-yellow-300 cursor-pointer rounded px-0.5 transition-colors"
          >
            {seg.text}
          </mark>
        ) : (
...
function TimerBadge({ seconds }: { seconds: number }) {
  ...
  const s   = seconds % 60
  const cls = seconds <= 30 ? 'text-red-600 bg-red-50' :
              seconds <= 60 ? 'text-orange-600 bg-orange-50' :
                              'text-gray-600 bg-gray-100'
  return (
    <span className={`flex items-center gap-1 text-sm font-mono font-semibold px-3 py-1 rounded-full ${cls}`}>
      <Clock size={13} />
    ...
    <div className="p-4 max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <button onClick={() => { setTimerActive(false); setPhase('select') }} className="btn-ghost p-2 rounded-xl">
            <ChevronLeft size={18} />
          </button>
          <div>
      ...
      {timeLeft === 0 && (
        <div className="card bg-red-50 border-red-100 mb-3">
          <p className="text-sm text-red-700 font-medium text-center">⏰ Vaqt tugadi! Mashqlarga o'ting.</p>
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-1.5">
          {text.vocabWords.map((v) => (
            <button
              ...
              className="text-xs bg-yellow-200 hover:bg-yellow-300 text-yellow-900 px-2 py-0.5 rounded-full transition-colors"
            >
              {v.word}
            </button>
          ))}
