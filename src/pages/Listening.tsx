        ...
        <div className="flex items-center gap-1 mb-6">
          {stepLabels.map((_lbl, i) => (
            <div key={i} className="flex-1 h-1.5 rounded-full">
              <div className={`h-full rounded-full ${i <= exStep ? 'bg-orange-400' : 'bg-gray-100'}`} />
            </div>
          ))}
              ...
              return (
                <div key={q.id} className="card">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-sm text-gray-800 leading-relaxed flex flex-wrap items-center gap-1">
                      <span>{parts[0]}</span>
                      <input
...
function TranscriptCard({ line }: { line: { startSec: number; text: string; speaker?: string } }) {
  ...
  return (
    <button
      onClick={() => setOpen((v) => !v)}
      className="w-full card text-left hover:bg-gray-50 transition-colors py-3"
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className={`text-sm ${open ? 'text-gray-900' : 'text-gray-600 line-clamp-1'}`}>
            {line.text}
          </span>
        </div>
