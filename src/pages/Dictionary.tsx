...
function AddWordModal({ open, onClose, userId, onAdded }: {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-5 sm:p-6 animate-in fade-in zoom-in-95">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-bold text-gray-900">Yangi so'z qo'shish</h2>
        ...
        <div className="space-y-3">
          <div>
            ...
            <input
              ...
              placeholder="hello"
              className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm outline-none focus:border-primary-400 focus:ring-2 focus:ring-primary-100 transition-all resize-none"
            />
          </div>
...
function WordCard({ word, userId, onDeleted }: { word: DictWord; userId?: string | null; onDeleted?: () => void }) {
  ...
  if (word.source === 'user' && !expanded) {
    return (
      <button
        ...
        className="w-full flex items-center justify-between gap-2 px-3 py-2 rounded-xl hover:bg-blue-50 transition-colors text-left group"
      >
        <div className="flex items-center gap-2 min-w-0">
          <span className="font-semibold text-gray-900 text-sm truncate">{word.english}</span>
  ...
  return (
    <div
      ...
      } ${word.source === 'user' ? 'border-blue-200 bg-blue-50/30' : ''}`}
      onClick={() => setExpanded(!expanded)}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <span className="font-bold text-gray-900 text-base">{word.english}</span>
            <span className={`badge text-[10px] ${LEVEL_BADGES[word.level] ?? 'bg-gray-100 text-gray-600'}`}>
              {word.level}
            </span>
            {word.source === 'user' && (
              <span className="text-[10px] bg-blue-100 text-blue-600 font-semibold px-1.5 py-0.5 rounded">Siz</span>
  ...
  return (
    <div className="p-3 sm:p-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-4 sm:mb-6">
        ...
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-gray-900">Lug'at</h1>
      ...
      <div className="flex gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-3 bg-white border border-gray-200 rounded-2xl
              outline-none text-sm text-gray-800 placeholder-gray-400
      ...
      {!loading && searched && results.length === 0 && (
        <div className="flex flex-col items-center justify-center min-h-[200px] text-center">
          ...
          <p className="text-sm text-gray-500">"{query}" bo'yicha hech narsa topilmadi</p>
          <p className="text-xs text-gray-400 mt-1">Boshqa so'z qidirib ko'ring</p>
        </div>
      )}
